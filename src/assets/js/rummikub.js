...
Rummikub.prototype.validateTile = function(param) {
  ...
  for (var idx in param) {
    if (param[idx] != null) {
      ...
      console.log(
        '\ncheck serialNumberValidateResult ---->  ' +
          serialNumberValidateResult
      );
      console.log(
        'check sameNumverValidateResult ----> ' + sameNumverValidateResult
      );
  ...
  for (let idx in group) {
    if (group[idx].isJoker == true) {
      if (group[Number(idx) - 1] != null) {
        group[idx] = new Tile(
          Number(group[Number(idx) - 1].score) + 1,
          group[Number(idx) - 1].color,
          false
        );
      } else {
        group[idx] = new Tile(
...
Rummikub.prototype.validateSameNumber = function(param) {
  ...
  if (colorSet.size == colorSetNotDuplicated.length && scoreSet.size == 1) {
    return true;
  } else {
    return false;
  }
...
User.prototype.toString = function() {
  return (
    ...
    this.registerYN +
    ' use : ' +
    JSON.stringify(this.use) +
    ' own : ' +
    JSON.stringify(this.own)
  );
};
...
export default { Rummikub, User, Tile };
